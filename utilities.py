...
class MSEsmooth(nn.modules.loss._Loss):
    ...
    def forward(self, results, target):
        ...
        add_penalty = torch.mean(torch.square(diff_pt(unwrap(pred_phase), device = self.device, dtype = self.dtype)))
        add_penalty = add_penalty/(add_penalty.clone().detach())
        add_penalty = self.c_factor*add_penalty*(MSE_sum.clone().detach())

        return MSE_sum + add_penalty
...
class MSEsmooth2(nn.modules.loss._Loss):
    ...
    def forward(self, results, target):
        ...
        add_penalty = torch.mean(torch.square(diff_pt(phase_unwraped, device = self.device, dtype = self.dtype)))
        add_penalty = add_penalty/(add_penalty.clone().detach())
        add_penalty = self.c_factor*add_penalty*(MSE_sum.clone().detach())
        ...
        return MSE_sum + add_penalty + smooth_penalty
    
    
class MSEFFT_1(nn.modules.loss._Loss):
    '''
    Classical MSE loss function with continuity and smoothness penalty for rapid changes of the transforming phase using integral of phase FFT.
    \"c_factor\" denotes ratio of the continuity penalty to the MSE, while \"s_factor"\" denotes the analogous ratio in case
    of smoothness penalty.
    '''

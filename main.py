...
pulse_2 = sa.gaussian_pulse((1545,1565), 1555, 3, x_type='freq')
pulse_2.x_type = "wl"
pulse_2.wl_to_freq()
...
pulse_2_conc_target_torch = torch.tensor(pulse_2_conc_target.reshape(1,signal_len*2), requires_grad=True, device=device_).to(dtype=dtype_)
...
for epoch in tqdm(range(config['epoch_num'])):
    ...
    #print(results.shape)
    x = torch.tensor([pulse_1.Y.real, pulse_1.Y.imag], requires_grad=True, device=device_).to(dtype=dtype_)
...
pulse_1.inv_fourier()   
sa.plot(pulse_1, title='po_1' , save=True)
sa.plot(pulse_2, title='target_2' , save=True)
